...
export async function predictMatch(homeTeam, awayTeam, stage) {
  ...
  const homeExpectedGoals = clamp(((homeX + awayXA) / 2) * baseGoalFactor * homeAdvantage * suppressionFactor, 0.5, 4.5);
  const awayExpectedGoals = clamp(((awayX + homeXA) / 2) * baseGoalFactor * suppressionFactor, 0.5, 4.5);
...
export async function predictMatchWithStats(homeTeam, awayTeam, stage, homeRecentStats, awayRecentStats) {
  ...
  if (xgResult.source === 'xg' && xgResult.homeExpectedGoals !== null) {
    homeExpectedGoals = clamp(xgResult.homeExpectedGoals * baseGoalFactor * homeAdvantage * suppressionFactor, 0.5, 4.5);
    awayExpectedGoals = clamp(xgResult.awayExpectedGoals * baseGoalFactor * suppressionFactor, 0.5, 4.5);
  } else {
    const fallback = getFallbackExpectedGoals(homeTeam.id, awayTeam.id, stage);
    homeExpectedGoals = clamp(fallback.homeExpectedGoals * baseGoalFactor * homeAdvantage * suppressionFactor, 0.5, 4.5);
    awayExpectedGoals = clamp(fallback.awayExpectedGoals * baseGoalFactor * suppressionFactor, 0.5, 4.5);
  }
