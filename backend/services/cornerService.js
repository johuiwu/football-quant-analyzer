...
export const DEFAULT_STRATEGIES = [
  { id: 1, enabled: false, name: "策略一 · 走地角球(35'-55')", market_type: "over_under", period: "full", minute_min: 35, minute_max: 55, leadGoals: 99, leadGoalsWeak: 0, line_min: 7.5, line_max: 11.5, odds_min: 0.8, odds_max: 1.2, direction: "Over", corner_min: 0, corner_max: 99, leadSide: "any", aiFilterEnabled: false },
  { id: 2, enabled: false, name: "策略二 · 领先让球(50'-77')", market_type: "handicap", period: "full", minute_min: 50, minute_max: 77, leadGoals: 3, leadGoalsWeak: 1, line_min: -1.5, line_max: 1.5, odds_min: 0.8, odds_max: 1.3, direction: "Auto", corner_min: 0, corner_max: 99, leadSide: "strong", aiFilterEnabled: false },
  { id: 3, enabled: false, name: "策略三 · 平局追角(70'-90')", market_type: "over_under", period: "full", minute_min: 70, minute_max: 90, leadGoals: 0, leadGoalsWeak: 0, line_min: 7.5, line_max: 10.5, odds_min: 0.6, odds_max: 0.9, direction: "Under", corner_min: 0, corner_max: 99, leadSide: "any", aiFilterEnabled: false },
  { id: 4, enabled: false, name: "策略四 · 领先大角(60'-80')", market_type: "over_under", period: "full", minute_min: 60, minute_max: 80, leadGoals: 2, leadGoalsWeak: 1, line_min: 7.5, line_max: 11.5, odds_min: 0.8, odds_max: 1.3, direction: "Over", corner_min: 0, corner_max: 99, leadSide: "strong", aiFilterEnabled: false },
  { id: 5, enabled: false, name: "策略五 · 尾声保底(75'-99')", market_type: "next_corner", period: "full", minute_min: 75, minute_max: 99, leadGoals: 99, leadGoalsWeak: 0, line_min: 0, line_max: 0, odds_min: 0.8, odds_max: 1.0, direction: "Auto", corner_min: 0, corner_max: 99, leadSide: "any", aiFilterEnabled: false },
  { id: 6, enabled: false, name: "策略六 · 逆风让球(55'-70')", market_type: "handicap", period: "full", minute_min: 55, minute_max: 70, leadGoals: 1, leadGoalsWeak: 1, line_min: -1.5, line_max: 1.5, odds_min: 0.9, odds_max: 1.3, direction: "Auto", corner_min: 0, corner_max: 99, leadSide: "weak", aiFilterEnabled: false },
  { id: 7, enabled: false, name: "策略七 · 均值回归(60'-80')", market_type: "over_under", period: "full", minute_min: 60, minute_max: 80, leadGoals: 99, leadGoalsWeak: 0, line_min: 7.5, line_max: 10.5, odds_min: 0.9, odds_max: 1.2, direction: "Over", corner_min: 3, corner_max: 5, leadSide: "any", aiFilterEnabled: false },
];
      ...
      try {
        ...
        if (betConfig.autoBetConfirmRequired) {
          ...
          if (genResult.success && !genResult.skipped) {
            // [CornerE2E] 节点5：投注队列生成（二次确认开启路径）
            console.debug(`[CornerE2E][节点5-投注队列生成] 策略${sid} 触发成功！方向: ${betDir}, 市场类型: ${strategy?.market_type || 'auto'}, 盘口周期: ${strategy?.period || 'full'}, 赔率: ${actualOdds}`);
            betQueue.push({
              betId: genResult.id,
              historyId: null,
...
async function pollOnce() {
  ...
  // [CornerE2E] 节点1：数据解析
  console.debug(`[CornerE2E][节点1-数据解析] 轮询获取到 ${matches.length} 场比赛`);
  if (matches.length > 0) {
    const s = matches[0];
    const hcpList = (s.handicaps || []).map(h => `${h.category} ${h.handicap ?? h.line ?? ''}`).join(', ');
    console.debug(`[CornerE2E][节点1-数据解析] 第一场样例: ${s.homeTeam || '?'} vs ${s.awayTeam || '?'}, 角球: ${s.homeCorners ?? '?'}-${s.awayCorners ?? '?'}, 盘口: [${hcpList || '无'}]`);
  }
