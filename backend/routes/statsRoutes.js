...
function buildResponse(team, stats, source) {
  ...
  return {
    ...
    stats: {
      ...
      homeXg: stats.xgFor && stats.xgFor.home ? parseFloat(stats.xgFor.home.toFixed(1)) : 0,
      awayXg: stats.xgFor && stats.xgFor.away ? parseFloat(stats.xgFor.away.toFixed(1)) : 0,
      homeStats: {
        played: homePlayed,
        wins: homeWins,
        draws: homeDraws,
...
function buildResponseFromDb(team, row, source) {
  ...
  return {
    ...
    stats: {
      ...
      shotsPerGame: matchesPlayed > 0 ? parseFloat((shotsTotal / matchesPlayed).toFixed(1)) : 0,
      shotAccuracy: shotsTotal > 0 ? parseFloat(((shotsOnTargetTotal / shotsTotal) * 100).toFixed(1)) : 0,
      homeXg: parseFloat((row.home_xg || 0).toFixed(1)),
      awayXg: parseFloat((row.away_xg || 0).toFixed(1)),
      homeStats: {
        played: homePlayed,
