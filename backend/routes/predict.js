...
router.post('/predict/poisson', requireFields(['teamA_id', 'teamB_id']), validateTypes({ teamA_id: 'number', teamB_id: 'number', isNeutral: 'boolean', stage: 'string' }), async (req, res) => {
  try {
    const { 
      teamA_id, 
      teamB_id, 
...
router.post('/predict/simulate', requireFields(['teamA_id', 'teamB_id']), validateTypes({ teamA_id: 'number', teamB_id: 'number', isNeutral: 'boolean', stage: 'string' }), validateRanges({ simulations: { min: 1, max: 10000 } }), async (req, res) => {
  try {
    const { 
      teamA_id, 
      teamB_id, 
    ...
    for (let i = 0; i < simulations; i++) {
      const result = simulatePoissonMatch(teamAStrength, teamBStrength, isNeutral, stage);
      results.push(result);
