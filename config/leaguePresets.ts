...
export interface LeaguePreset {
  key: string;                  // 联赛唯一标识 (与 ALL_LEAGUE_TEAMS.leagueKey 对齐)
  nameCn: string;               // 中文名
  matchesPerSeason: number;     // 每赛季总比赛数
  crawlerSlug?: string;         // qiumiwu.com 联赛 slug (有则支持积分榜爬取)
  maxTeams?: number;            // 联赛固定球队数量（用于爬虫结果过滤）
  teamSlugs?: Record<string, string>;  // 球队中文名 → qiumiwu slug (有则支持统计爬取)
}

export const LEAGUE_PRESETS: Record<string, LeaguePreset> = {
  EPL: {
...
// 获取某联赛每赛季比赛数（兜底用）
export function getMatchesPerSeason(leagueKey: string): number {
