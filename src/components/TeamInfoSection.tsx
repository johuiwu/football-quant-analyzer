...
export default function TeamInfoSection() {
  ...
  const teamProfile = useMemo(() => {
    ...
    return `### ${t.nameCn} 战术特征画像\n*(基于 ${t.leagueCn} 2025/2026 赛季数据)*\n\n**1. 技战术底盘：**\n场均可轰出 **${t.shotsPerGame} 次射门**，精度 **${t.shotAccuracy}%**，零封 **${t.cleanSheets} 场**。\n\n**2. 主客场抗性：**\n- 主场胜率 **${((t.homeStats.wins/hp)*100).toFixed(0)}%**，场均进球 **${(t.homeStats.goalsFor/hp).toFixed(1)}个**，xG净差 **${(t.homeStats.xgFor-t.homeStats.xgAgainst).toFixed(1)}球**\n- 客场胜率 **${((t.awayStats.wins/ap)*100).toFixed(0)}%**，场均进球 **${(t.awayStats.goalsFor/ap).toFixed(1)}个**，xG净差 **${(t.awayStats.xgFor-t.awayStats.xgAgainst).toFixed(1)}球**\n\n**3. 近期走势：** ${t.form.join(' → ')}。`;
  }, [safeTeam, activeTeam]);
  ...
  const hxd = safeTeam.homeStats.xgFor - safeTeam.homeStats.xgAgainst, axd = safeTeam.awayStats.xgFor - safeTeam.awayStats.xgAgainst;
            ...
            <>
              ...
              <div className="grid grid-cols-12 gap-5">
                <div className="col-span-7 space-y-4">
                  {[['🏠 主场 (HOME)', 'blue' as const, safeTeam.homeStats, hwp, hxd] as const, ['✈️ 客场 (AWAY)', 'orange' as const, safeTeam.awayStats, awp, axd] as const].map(([title, c, s, wp, xd]) => (
                    <div key={title} className="bg-[#0b0b0f] rounded-2xl border border-[#2a2a30] p-4 space-y-3">
                      <div className="flex items-center justify-between"><h4 className="text-[11px] font-bold text-slate-300 uppercase tracking-wider flex items-center gap-1.5"><MapPin className={`w-3.5 h-3.5 text-${c}-400`} /> {title}</h4><div className="text-right"><span className="text-[11px] text-slate-300 font-mono">{s.wins}胜 {s.draws}平 {s.losses}负</span><span className="text-[10px] text-emerald-400 ml-2 font-mono">胜率：{wp.toFixed(0)}%</span></div></div>
                      <div><div className="flex justify-between text-[10px] text-slate-500 mb-1"><span>战力指数</span><span>{Math.round(wp)}%</span></div><div className="w-full bg-slate-950 h-2.5 rounded-full overflow-hidden"><div className={`h-2.5 rounded-full bg-gradient-to-r ${c === 'blue' ? 'from-blue-500 to-cyan-400' : 'from-orange-500 to-amber-400'}`} style={{ width: `${Math.min(wp, 100)}%` }} /></div></div>
                      <div className="grid grid-cols-2 gap-3 pt-1"><div><span className="block text-[10px] text-slate-500">xG 期望进球</span><span className="text-base font-bold text-emerald-400 font-mono">{s.xgFor.toFixed(2)}</span></div><div><span className="block text-[10px] text-slate-500">xG 期望失球</span><span className="text-base font-bold text-rose-400 font-mono">{s.xgAgainst.toFixed(2)}</span></div></div>
                      <div className="pt-1 border-t border-[#2a2a30]"><span className="text-[10px] text-slate-500">xG 净差值</span><span className={`text-base font-bold font-mono ml-2 ${xd >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{xd >= 0 ? '+' : ''}{xd.toFixed(2)}</span></div>
                    </div>
                  ))}
                </div>
