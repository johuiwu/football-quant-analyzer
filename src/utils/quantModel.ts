...
export interface AggregatedDecision {
  ...
  direction: 'HOME_WIN' | 'DRAW' | 'AWAY_WIN' | 'OVER' | 'UNDER';
...
function calculateDynamicDrawProb(homeStrength: number, awayStrength: number): number {
  ...
  const raw = 0.25 * Math.exp(-diff * 2);
...
odds1X2 = odds ?? { home: 2.0, draw: 3.2, away: 3.5 };
  ...
  // P3-16: cache check
  const ck = JSON.stringify({ hi: homeTeam.id, ai: awayTeamVal.id, o: odds1X2, af: asianFeatures, gl: finalGoalsLine, w: finalWeights, ap: finalAdvancedParams, fw: finalFusionWeights });
  const ch = mc.get(ck);
  if (ch) return ch;
...
const homeExt = extractExtendedFeatures(homeTeam, league);
  ...
  const awayStrength = statsAwayStrength * 0.6 + (1 - eloHomeWinExpectancy) * 2.0 * 0.4;
...
const homeXgDiff = (homeTeam.homeStats.xgFor - homeTeam.homeStats.xgAgainst) / homeHPlayed;

  const awayXgDiff = (awayTeamVal.awayStats.xgFor - awayTeamVal.awayStats.xgAgainst) / awayAPlayed;
...
const strengthDiff = homeStrength - awayStrength;
  ...
  strDrawProb = Math.max(0.14, Math.min(0.40, strDrawProb));
  ...
  const totalHA = haHomeWinsWeighted + haAwayWinsWeighted + 0.5;
  ...
  const extFactor = (extMomentumHome - extMomentumAway) + (extDefHome - extDefAway) + possessionAdvantage;
  ...
  let compDraw = (
    ...
    calculateDynamicDrawProb(homeStrength, awayStrength) * weights.h2h +

    calculateDynamicDrawProb(homeStrength, awayStrength) * weights.form
  ...
  const calculateAggregatedDecision = (
    ...
    let finalDirection: 'HOME_WIN' | 'DRAW' | 'AWAY_WIN' | 'OVER' | 'UNDER';
    ...
    if (underBasicCondition && (underHighConfidence || underVeryLowGoals || underModelsAgree)) {
      ...
      finalConfidence = Math.min(0.95, 0.5 + (lowScoreProbability.totalLowScore - 0.5) * 1.5);

    }

    else if (eloDiffVal > 0.2) {

      finalDirection = eloType as 'HOME_WIN' | 'DRAW' | 'AWAY_WIN';

      finalConfidence = Math.min(0.95, eloValue * eloWeight + (poissonType.includes(eloType) || poissonType === 'NEUTRAL' ? poissonValue * poissonWeight : 0) + (dixonColesType !== 'OVER' ? dixonColesValue * dixonColesWeight : 0));

    }

    else if ((poissonType === 'OVER' && dixonColesType === 'OVER') || (poissonType === 'UNDER' && dixonColesType === 'UNDER')) {

      finalDirection = poissonType;

      finalConfidence = Math.min(0.95, (poissonValue * poissonWeight + dixonColesValue * dixonColesWeight) / (poissonWeight + dixonColesWeight));
  ...
  let recommendedDirection = '平局 / 观望';
...
export function calculateBayesianLiveUpdate(
  ...
  for (let rH = 0; rH <= maxGoals; rH++) {
    ...
    for (let rA = 0; rA <= maxGoals; rA++) {
      ...
      const cellDixon = dixonColesAdjustment(rH, rA, remainingExpectedHomeGoals, remainingExpectedAwayGoals, -0.05);
